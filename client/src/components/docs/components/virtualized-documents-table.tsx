...
// Memoized row heights
const ROW_HEIGHT_MOBILE = 85;
const ROW_HEIGHT_DESKTOP = 52;

// Sharing status badge component - memoized
const SharingStatusBadge = memo(function SharingStatusBadge({
  doc,
  compact = false,
...
// Memoized document row for mobile
const MobileDocumentRow = memo(function MobileDocumentRow({
  doc,
  style
}: {
  doc: DocumentWithOwner;
...
// Memoized document row for desktop
const DesktopDocumentRow = memo(function DesktopDocumentRow({
  doc,
  style
}: {
  doc: DocumentWithOwner;
